...
const page = ({ params }) => {
            ...
            .then((snapshot) => {
                if (snapshot.exists()) {
                    ...
                    if (studentData) {
                        ...
                        setClgID(studentData.idImage);
                        setResult(studentData.CurrentImage);
                    }
                } else {
                    if (isMounted) {
    ...
    const handleSubmit = async (e) => {
        ...
        // } else 
        if (inputImage.files[0] && inputID.files[0]) {
            ...
            const studentsRef = ref(db, `Students/${id}`)
            const storage = getStorage(app);
            const storageRef = Sref(storage, '/Images' + `/${id}.png`)
            const storageIDRef = Sref(storage, '/ID' + `/${id}.png`)
                ...
                <form onSubmit={handleSubmit}>
                    <div className='formWrapper'>
                        ...
                        <div className='geS5n'>
                            ...
                            <div className='field'>
                                <input id="inputImage" accept="image/*" type="file" name="image" hidden />
